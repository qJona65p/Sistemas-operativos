...
void imprimeProcEjec(Proceso *tempProceso){
    ...
    gotoxy(XPE+14, 10); cout << tempProceso->getQt();
    gotoxy(XPE+14, 11); cout << tempProceso->getTEM();
}
void imprimeProcBCP(int x, int y, int contG, Proceso tempProceso){
    gotoxy(x+33, y); cout << "x"; gotoxy(x+66, y); cout << "-";
...
void admision(int Q, listaDL <Proceso> *nuevos, listaDL <Proceso> *listos, listaDL <Proceso> *bloq, listaDL <Proceso> *fin, int *contG, int *MOD, int proNum){
    Proceso tempProceso;
...
int ejecutaProceso(int Q, listaDL <Proceso> *nuevos, listaDL <Proceso> *listos, listaDL <Proceso> *bloq, listaDL <Proceso> *fin, Proceso *tempProceso, int proNum, int *contG, int *MOD){
    ...
    for (int i=1; i <= restante; i++) {
        ...
        gotoxy(XPE+14, 9); cout << tempProceso->getTTr();
        gotoxy(XPE+14, 10); cout << i + tempProceso->getQt();
        gotoxy(XPE+14, 11); cout << tempProceso->getTEM()-tempProceso->getTTr() << "  ";
        for (int i=0; i < bloq->getTamanio(); i++) {
        ...
        if (kbhit()) {
            ...
            if ((tecla == 'b' || tecla == 'B') && (tempProceso->getTEM() != tempProceso->getTTr())) {tempProceso->setQt(i); return 2;}
            if (tecla == 'n' || tecla == 'N') {
                nuevos->push_b(generaProceso(nuevos, 1+proNum+nuevos->getTamanio()+listos->getTamanio()+bloq->getTamanio(), *contG));
                admision(Q, nuevos, listos, bloq, fin, contG, MOD, proNum);
                imprimeListos(listos);
            }
        }
        if ((i == Q-tempProceso->getQt()) && (tempProceso->getTEM() != tempProceso->getTTr())) return 3;
    }
    YPF=6+proNum+7*pos;
...
void ejecucion(listaDL <Proceso> *nuevos, int Q){
    ...
    while(!nuevos->vacia() || !bloq.vacia()){
        admision(Q, nuevos, &listos, &bloq, &final, &contGlobal, &MOD, proNum);
        while (!listos.vacia()) {
            didIn=0;
            gotoxy(21, 1); cout << nuevos->getTamanio() << " ";
            tempProceso=listos.peek_p(0);
            tempProceso.setEstado(2, contGlobal);
            didIn=ejecutaProceso(Q, nuevos, &listos, &bloq, &final, &tempProceso, proNum++, &contGlobal, &MOD);
            listos.pop_f();
...
int main(){
    ...
    Ventanas(0, 0, 121, 30, 1, "Algoritmo de Planificacion RR (Round-Robin) ");
    gotoxy(2, 1); cout << "Programa 6";
    ...
    gotoxy(3, 8); cout << "->"; pt=intCheck(3, 8, 3, 1, 100, "**Rango 1 - 100");
    ejecucion(&nuevos, pt);
